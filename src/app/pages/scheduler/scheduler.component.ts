...
export class SchedulerComponent implements AfterViewChecked {
  ...
  //   sourceFiles.files = ['./scheduler.component.scss'];
  // }
  ...
  public onSettingsClick(args): void {
    const settingsPanel: Element | null = document.querySelector('.overview-content .right-panel');
  ...
  public getWeatherImage(value: Date): string | null {
    switch (value.getDay()) {
      case 0:
  ...
  async fetchTrademarks() {
    ...
    await this.configService.getTrademarks().subscribe((res: any) => {
      let tempList = res || [];
      let tList = [];
  ...
  public generateEvents(): Record<string, any>[] {
    ...
    for (let i = 0; i < this.trademarks.length; i++) {
      ...
      const start: Date = new Date(this.trademarks[i].dateOfExpiry);
      const end: Date = new Date(start.getTime());
      end.setHours(end.getHours() + 2);
      startDate = new Date(start.getTime());
      endDate = new Date(end.getTime());
      eventData.push({
        Id: this.trademarks[i].id,
        Subject: this.trademarks[i].titleOfArtwork,
        date: new Date(this.trademarks[i].dateOfExpiry),
